...
// API Base URL - .env'den al veya direkt yaz
const API_BASE_URL = 'https://localhost:7001/api'; // Senin API URL'in
...
const tourService = {
  ...
  uploadImage: async (file) => {
    ...
    const response = await fetch(`${API_BASE_URL}/FileUpload/upload-image`, {
      method: 'POST',
      body: formData
    });
    return response.json();
...
const AdminToursDashboard = () => {
  ...
  const handleEdit = (tour) => {
    const trData = tour.tourTranslations?.find(t => t.languageCode === 'tr-TR') || {};
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire('Hata!', 'Tur silinirken hata oluştu', 'error');
      }
    }
  };
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-48">
                        <img
                          src={tour.mainImage || 'https://via.placeholder.com/400x300?text=No+Image'}
                          alt={trData.title}
                      ...
                      <div className="p-5">
                        <h3 className="text-xl font-bold text-slate-800 mb-2">{trData.title}</h3>
                        <p className="text-slate-600 text-sm mb-4 line-clamp-2">{trData.description}</p>
          ...
          {(activeTab === 'new' || activeTab === 'edit') && (
            ...
            >
              <h2 className="text-2xl font-bold text-slate-800 mb-6">
                {activeTab === 'new' ? 'Yeni Tur Ekle' : 'Turu Düzenle'}
              </h2>
